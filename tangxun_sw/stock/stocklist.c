...
static tx_stock_info_t ls_tsiStockInfoIndex[] =
{
...
static olint_t ls_nNumOfStockInfoIndex =
    sizeof(ls_tsiStockInfoIndex) / sizeof(tx_stock_info_t);

/* --- private routine section ------------------------------------------------------------------ */
static u32 _findStock(tx_stock_list_t * ptsl, olchar_t * name, tx_stock_info_t ** info)
{
    ...
    if (u32Ret != JF_ERR_NO_ERROR)
    {
        ...
        {
            if (strncmp(name, ls_tsiStockInfoIndex[i].tsi_strCode,
                        ol_strlen(ls_tsiStockInfoIndex[i].tsi_strCode)) == 0)
            {
                *info = &ls_tsiStockInfoIndex[i];
                u32Ret = JF_ERR_NO_ERROR;
                break;
            }
        }
...
static u32 _getStockIndustry(
    jf_string_parse_result_field_t * field, tx_stock_info_t * stockinfo)
{
    u32 u32Ret = JF_ERR_NO_ERROR;
    tx_stock_indu_info_t * desc;
    olint_t i, j;
    ...
    for (i = 0; i < ls_u32NumberOfInduInfo; i ++)
    {
        ...
        if (desc->tsii_nChineseDesc == field->jsprf_sData)
        {
            for (j = 0; j < desc->tsii_nChineseDesc; j ++)
            {
                if (desc->tsii_strChinese[j] != field->jsprf_pstrData[j])
                    break;
            }

            if (j == desc->tsii_nChineseDesc)
            {
    ...
    if (stockinfo->tsi_nIndustry == 0)
    {
        ...
        jf_logger_logErrMsg(
            u32Ret, "Unknown industry for stock %s", stockinfo->tsi_strCode);
    }
    ...
    olchar_t * line, olsize_t sline, tx_stock_info_t * stockinfo)
{
    ...
    memset(stockinfo, 0, sizeof(tx_stock_info_t));
    ...
    if (u32Ret == JF_ERR_NO_ERROR)
    {
        ...
        if (field->jsprf_pstrData[0] != '-')
            u32Ret = jf_string_getU64FromString(
                ...
                &stockinfo->tsi_u64GeneralCapital);
    }
    ...
    if (u32Ret == JF_ERR_NO_ERROR)
    {
        ...
        if (field->jsprf_pstrData[0] != '-')
            u32Ret = jf_string_getU64FromString(
                ...
                &stockinfo->tsi_u64TradableShare);
    }
...
static u32 _classifyStock(tx_stock_list_t * ptsl)
{
    u32 u32Ret = JF_ERR_NO_ERROR;
         ...
         i ++)
    {
        ...
        sbuf = info->tsii_nStock * 9 + 1;

        u32Ret = jf_mem_alloc((void **)&info->tsii_pstrStocks, sbuf);
        if (u32Ret == JF_ERR_NO_ERROR)
        {
            info->tsii_pstrStocks[0] = '\0';
        }
    }

    stockinfo = ptsl->tsl_ptsiStock;
    for (i = 0; i < ptsl->tsl_nNumOfStock; i ++)
    {
        info = &ls_tsiiInduInfo[stockinfo->tsi_nIndustry - 1];
        ol_strcat(info->tsii_pstrStocks, stockinfo->tsi_strCode);
        ol_strcat(info->tsii_pstrStocks, " ");
...
u32 initStockList(void)
{
    ...
    memset(ptsl, 0, sizeof(tx_stock_list_t));

    ptsl->tsl_nMaxStock = MAX_STOCKS;
    size = sizeof(tx_stock_info_t) * ptsl->tsl_nMaxStock;
    u32Ret = jf_mem_alloc((void **)&ptsl->tsl_ptsiStock, size);

    if (u32Ret == JF_ERR_NO_ERROR)
        u32Ret = _readStockList(STOCK_LIST_FILE, ptsl);

    if (u32Ret == JF_ERR_NO_ERROR)
        u32Ret = _classifyStock(ptsl);
...
u32 finiStockList(void)
{
    ...
    {
        jf_mem_free((void **)&ptsl->tsl_ptsiStock);
    }
    ...
    for (i = 0; i < ls_u32NumberOfInduInfo; i ++)
    {
        ...
        if (info->tsii_pstrStocks != NULL)
            jf_mem_free((void **)&info->tsii_pstrStocks);
    }
...
tx_stock_info_t * getNextStockInfoIndex(tx_stock_info_t * info)
{
    ...
    if (info < &ls_tsiStockInfoIndex[0] + ls_nNumOfStockInfoIndex - 1)
        ptsi = info + 1;
...
tx_stock_info_t * getStockInfoIndex(olchar_t * name)
{
    ...
    for (i = 0; i < ls_nNumOfStockInfoIndex; i ++)
    {
        if (ol_strncmp(ls_tsiStockInfoIndex[i].tsi_strCode, name, 8) == 0)
            return &ls_tsiStockInfoIndex[i];
    }
...
boolean_t isStockInfoIndex(olchar_t * code)
{
    ...
    tx_stock_info_t * ptsi = &ls_tsiStockInfoIndex[0];
    olint_t i;
...
u32 getStockInfo(olchar_t * name, tx_stock_info_t ** info)
{
    *info = NULL;
    ...
    return _findStock(&ls_tslStockList, name, info);
}
